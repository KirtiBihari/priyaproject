...
const canAddTokenToEndOfRegion = (
  ...
  if (
    ...
    return false;
  if (prevToken === TOKEN_TYPE.NUMBER) return false;
  return true;
};
...
const getTokenType = chunk => {
  ...
  if (typeof parseInt(chunk) === 'number') return TOKEN_TYPE.NUMBER;
};

export default (text, options) => {
  const tokens = text.split(/(k|K\w+|\s|[[:punct:]])/i).reduce((acc, chunk) => {
    const unfuzzyChunk =
