        ...
        tokens
          .reduce((acc, token, i) => {
            ...
            if (token.type === TOKEN_TYPE.NUMBER) {
              return acc.concat({
                token,
                numberValue: parseInt(token.lowerCaseValue)
              });
            }
            return acc.concat({
              token,
...
export default ({ regions, textz }) => {
  if (!regions) return textz;
  if (regions[0].end - regions[0].start === textz.length - 1)
    return getNumber(regions[0]);
  return replaceRegionsInText(regions, textz);
};
